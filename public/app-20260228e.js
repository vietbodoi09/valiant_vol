...
async function fetchValiantCLOB(walletAddress, startTime, endTime) {
  try {
    ...
    for (const pair of pairs) {
      // Use limit=100 like the UI does
      const url = `https://browser-api.valiant.trade/market-api/wallet-trades?wallet=${walletAddress}&limit=100&provider=codex&pair_address=${pair.address}`;
      
      try {
        addLogEntry('info', `🟣 Fetching CLOB ${pair.name}...`);
        const res = await fetchWithTimeout(url, {}, 15000);
        
        if (!res.ok) {
          console.log(`CLOB ${pair.name} response:`, res.status);
          continue;
        }
        
        const data = await res.json();
        console.log(`CLOB ${pair.name} data:`, data);
        
        // Response might be wrapped
        const trades = Array.isArray(data) ? data : (data.data || data.trades || []);
        
        if (!Array.isArray(trades) || trades.length === 0) {
          addLogEntry('info', `🟣 CLOB ${pair.name}: 0 trades`);
          continue;
        }
        ...
        for (const trade of trades) {
          // Time filter
          if (trade.blockUnixTime < startTime || trade.blockUnixTime > endTime) continue;
        ...
        addLogEntry('info', `🟣 CLOB ${pair.name}: ${count}/${trades.length} trades in range`);
      } catch (e) {
        console.error(`CLOB ${pair.name} error:`, e);
        addLogEntry('warning', `⚠️ CLOB ${pair.name}: ${e.message}`);
      }
    }
