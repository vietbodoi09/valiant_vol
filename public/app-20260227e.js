...
async function fetchFogoscanWalletData(walletAddress, startTime, endTime) {
  ...
  if (total != null) addLogEntry('info', `📊 Total Valiant swaps: ${Number(total).toLocaleString()}`);
  ...
  // Fetch pages in batches for speed
  while (!reachedStart && currentPage <= CONFIG.FOGOSCAN_MAX_PAGES) {
    if (abortController?.signal.aborted) throw new Error('Aborted');

    // Create batch of page requests
    const batchPages = [];
    for (let i = 0; i < BATCH_SIZE && currentPage + i <= CONFIG.FOGOSCAN_MAX_PAGES; i++) {
      batchPages.push(currentPage + i);
    }

    setLoading(true, `🔵 Fetching pages ${currentPage}-${currentPage + batchPages.length - 1}... (${allSwaps.length} matches)`);

    // Fetch all pages in parallel
    const responses = await Promise.allSettled(batchPages.map(p => fetchFogoscanPage(p)));
    
    for (let i = 0; i < responses.length; i++) {
      const result = responses[i];
      const pageNum = batchPages[i];
      
      if (result.status !== 'fulfilled' || !result.value?.success || !result.value.data?.length) {
        reachedStart = true;
        break;
      }
      ...
      // Merge metadata tokens
      if (response.metadata?.tokens) {
        Object.assign(metaTokens, response.metadata.tokens);
      }

      // Sort by block_time descending (newest first)
      const sortedData = response.data.sort((a, b) => b.block_time - a.block_time);
      
      for (const item of sortedData) {
        const blockTime = item.block_time;
        checkedCount++;

        // Early termination: if all items in this page are before startTime
        if (blockTime < startTime) {
          // Only stop if we're sure no more data (check if all remaining are also old)
          if (!startTimeReached) {
            startTimeReached = true;
            // Continue checking this page but mark for exit
          } else {
            reachedStart = true;
            break;
          }
        ...
        if (blockTime < startTime || blockTime > endTime) continue;
        if (item.from_address !== targetWallet) continue;
