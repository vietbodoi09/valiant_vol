...
// ============================================
// VALIANT CLOB API - with pagination
// ============================================
async function fetchValiantCLOB(walletAddress, startTime, endTime) {
  try {
    const pairs = [
      { address: 'J7mxBLSz51Tcbog3XsiJTAXS64N46KqbpRGQmd3dQMKp', name: 'FOGO-USDC' },
      { address: 'HULdR8aMSxJAiNJmrTBcfKN4Zq6FgG33AHbQ3nDD8P5E', name: 'FOGO-iFOGO' },
    ];
    ...
    for (const pair of pairs) {
      ...
      while (hasMore && page <= 20) { // Max 2000 trades per pair
        try {
          const url = `https://browser-api.valiant.trade/market-api/wallet-trades?wallet=${walletAddress}&limit=100&page=${page}&provider=codex&pair_address=${pair.address}`;
          
          addLogEntry('info', `🟣 CLOB ${pair.name} page ${page}...`);
          const res = await fetchWithTimeout(url, {}, 15000);
          
          if (!res.ok) {
            console.log(`CLOB ${pair.name} page ${page} response:`, res.status);
            break;
          }
          ...
          for (const trade of trades) {
            // Stop if trade is too old (data is sorted newest first)
            if (trade.blockUnixTime < startTime) {
              hasMore = false;
              break;
            }
            
            // Skip if after end time
            if (trade.blockUnixTime > endTime) continue;
            ...
            const tx = {
              ...
              tokenA: trade.fromSymbol,
              tokenB: trade.toSymbol,
              amountA: trade.fromAmount,
              amountB: trade.toAmount,
              direction: trade.txType === 'buy' ? 'BtoA' : 'AtoB',
              usdVolume: parseFloat(trade.volumeUsd) || 0,
              fogoVolume: (parseFloat(trade.volumeUsd) || 0) / fogoPrice,
              signer: trade.owner,
              source: 'clob',
              isValiant: true,
          ...
          addLogEntry('info', `🟣 CLOB ${pair.name} page ${page}: ${inRangeCount} in range (${trades.length} total)`);
          ...
          console.error(`CLOB ${pair.name} page ${page} error:`, e);
          break;
        }
      }
      
      addLogEntry('info', `🟣 CLOB ${pair.name}: ${totalFetched} total fetched`);
    }
    
    return allTrades;
  } catch (e) {
    console.error('Valiant CLOB error:', e);
    return [];
  }
}

// Fetch wallet data using 'from' filter with PARALLEL requests (ultra fast!)
async function fetchFogoscanWalletDirect(walletAddress, startTime, endTime) {
  try {
    const PAGE_SIZE = 100; // API limit
    const CONCURRENT_BATCH = 10; // Fetch 10 pages at once
    let allData = [];
    let metaTokens = {};
    let currentPage = 1;
    ...
    addLogEntry('info', `🔵 Fetching wallet data in parallel batches...`);
    
    while (!shouldStop && currentPage <= 200) { // Max 200 pages = 20k txs
      ...
      for (let i = 0; i < CONCURRENT_BATCH; i++) {
        ...
        const url = `${FOGOSCAN_API}/defi/amm/activity?app_id=${VALIANT_APP_ID}&from=${walletAddress}&page=${pageNum}&page_size=${PAGE_SIZE}`;
        batchPromises.push(
      ...
      setLoading(true, `🔵 Fetching pages ${currentPage}-${currentPage + CONCURRENT_BATCH - 1}... (${allData.length} txs)`);
      
      const results = await Promise.all(batchPromises);
      
      let batchOldestTime = Infinity;
      ...
      for (const json of results) {
        ...
        for (const item of json.data) {
          batchOldestTime = Math.min(batchOldestTime, item.block_time);
          
          // Time filter
          if (item.block_time < startTime) {
            shouldStop = true;
            continue;
          }
          if (item.block_time > endTime) continue;
      ...
      // Stop conditions
      if (!foundDataInBatch) shouldStop = true;
      if (batchOldestTime < startTime) shouldStop = true;
      
      currentPage += CONCURRENT_BATCH;
    }
    
    addLogEntry('info', `🔵 Fetched ${totalFetched} total, kept ${allData.length} in range`);
    return { data: allData, metaTokens, totalPages: Math.ceil(currentPage / CONCURRENT_BATCH) };
  } catch (e) {
...
async function fetchVolumeData() {
  ...
  try {
    ...
    // ① Try Valiant CLOB (Order Book) trades
    let clobTrades = [];
    try {
      addLogEntry('info', '🟣 Fetching Valiant CLOB (Order Book) trades...');
      clobTrades = await fetchValiantCLOB(walletAddress, startTime, endTime);
      if (clobTrades.length > 0) {
        addLogEntry('success', `✅ CLOB: ${clobTrades.length} order book trades`);
      }
    } catch (e) {
      addLogEntry('warning', `⚠️ CLOB error: ${e.message}`);
    }

    // ② Try Fogoscan AMM (for recent data with USD values)
    try {
      fogoscanData = await fetchFogoscanWalletData(walletAddress, startTime, endTime);
      if (fogoscanData && fogoscanData.totalSwaps > 0) {
        addLogEntry('success', `✅ Fogoscan: ${fogoscanData.totalSwaps} AMM swaps`);
      } else {
    ...
    // ③ Always fetch RPC for complete history
    ...
    // ④ Merge all sources: CLOB + Fogoscan + RPC
    let data = mergeAllSources(clobTrades, fogoscanData, rpcData, startTime, endTime);
...
// MERGE ALL SOURCES: CLOB + FOGOSCAN + RPC
// ============================================
function mergeAllSources(clobTrades, fogoscanData, rpcData, startTime, endTime) {
  const txMap = new Map();
  
  // Add CLOB trades (highest priority - has exact USD values)
  for (const tx of clobTrades) {
    txMap.set(tx.signature, { ...tx, source: 'clob' });
  }
  
  // Add Fogoscan AMM data
  if (fogoscanData?.transactions) {
  ...
  // Add RPC data (fill gaps)
  if (rpcData?.transactions) {
    for (const tx of rpcData.transactions) {
      if (!txMap.has(tx.signature)) {
        txMap.set(tx.signature, { ...tx, source: 'rpc' });
      }
    }
  }
  
  const merged = [...txMap.values()].sort((a, b) => b.timestamp - a.timestamp);
  
  const totalUsd = merged.reduce((s, t) => s + (t.usdVolume || 0), 0);
  const totalFogo = merged.reduce((s, t) => s + (t.fogoVolume || 0), 0);
  ...
  const sources = [];
  if (clobTrades.length) sources.push('clob');
  if (fogoscanData?.totalSwaps) sources.push('fogoscan');
  if (rpcData?.totalSwaps) sources.push('rpc');
  
  addLogEntry('info', `🟢 Merged: ${clobTrades.length} CLOB + ${fogoscanData?.totalSwaps || 0} Fogoscan + ${rpcData?.totalSwaps || 0} RPC = ${merged.length} total`);
  ...
  return {
    ...
    transactions: merged,
    onlyValidPools: true,
    poolsUsed: [...new Set(merged.map(t => t.pool))],
    source: sources.length > 1 ? `merged (${sources.join('+')})` : sources[0] || 'none',
  };
}

// ============================================
// MERGE FOGOSCAN + RPC DATA (legacy)
// ============================================
