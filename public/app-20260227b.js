...
const FOGOSCAN_API  = 'https://api.fogoscan.com/v1';
 ...
 * }
 */
async function fetchFogoscanPage(page, pageSize = CONFIG.FOGOSCAN_PAGE_SIZE) {
  try {
    const url = `${FOGOSCAN_API}/defi/amm/activity?app_id=${VALIANT_APP_ID}&page=${page}&page_size=${pageSize}`;
...
async function fetchFogoscanWalletData(walletAddress, startTime, endTime) {
  ...
  // Normalize wallet address (Solana addresses are base58, case-sensitive)
  const targetWallet = walletAddress.trim();
  addLogEntry('info', `🔵 Looking for wallet: ${targetWallet.slice(0, 16)}...`);

  // Show total swap count
  const total = await fetchFogoscanTotal();
  if (total != null) addLogEntry('info', `📊 Total Valiant swaps ever: ${Number(total).toLocaleString()}`);
  ...
  while (!reachedStart && page <= CONFIG.FOGOSCAN_MAX_PAGES) {
    if (abortController?.signal.aborted) throw new Error('Aborted');

    setLoading(true, `🔵 Fogoscan page ${page}... (${allSwaps.length} matches)`);
    const response = await fetchFogoscanPage(page);

    if (!response?.success || !response.data?.length) break;
    
    // Debug: Log first item to see structure
    if (page === 1 && response.data[0]) {
      console.log('🔵 Fogoscan sample item:', response.data[0]);
      addLogEntry('info', `🔵 Sample from_address: ${response.data[0].from_address?.slice(0, 20) || 'N/A'}...`);
    }

    // Merge metadata tokens (for symbol/price lookup)
    if (response.metadata?.tokens) {
      Object.assign(metaTokens, response.metadata.tokens);
    }
    ...
    for (const item of response.data) {
      ...
      // Stop when we've gone past the start of the date range
      if (blockTime < startTime) { reachedStart = true; break; }

      // Skip swaps after end date
      if (blockTime > endTime) continue;

      // Filter by wallet address (case-sensitive on chain)
      const itemWallet = item.from_address;
      if (itemWallet !== targetWallet) continue;
      ...
      if (allSwaps.length % 20 === 0) updateLogStats(allSwaps.length, 0, 0, true);
    }
...
async function fetchFOGOPrice(rpcUrl) {
  ...
  try {
    const res = await fetchWithTimeout(
      `${FOGOSCAN_API}/defi/amm/activity?app_id=${VALIANT_APP_ID}&page=1&page_size=1`, {}, 8000
    );
    if (res.ok) {
      const json = await res.json();
...
async function fetchVolumeData() {
  ...
  try {
    ...
    // ① Try Fogoscan API
    try {
      data = await fetchFogoscanWalletData(walletAddress, startTime, endTime);
      if (!data || data.totalSwaps === 0) {
        addLogEntry('warning', '⚠️ Fogoscan: 0 matches — falling back to Fogo RPC');
        data = null;
      } else {
        addLogEntry('success', `✅ Fogoscan: ${data.totalSwaps} swaps (checked ${data.checkedCount || '?'})`);
      }
    } catch (e) {
      addLogEntry('warning', `⚠️ Fogoscan error: ${e.message} — using RPC`);
      data = null;
    }

    // ② RPC fallback
    if (!data) {
      addLogEntry('info', '🟡 Fetching via Fogo RPC...');
      data = await fetchAllTransactionsRPC(walletAddress, startTime, endTime, rpcUrl);
    }

    const elapsed = ((performance.now() - t0) / 1000).toFixed(1);
    flushLogBuffer();
    const badge = data.source === 'fogoscan' ? '🔵 Fogoscan API' : '🟡 Fogo RPC';
...
function displayResults(data) {
  ...
  if (!data.transactions?.length) {
    ...
    const frag  = document.createDocumentFragment();
    const limit = Math.min(data.transactions.length, 500);
    for (let i = 0; i < limit; i++) {
      const tx    = data.transactions[i];
      const input  = tx.direction==='AtoB' ? `${tx.amountA.toFixed(4)} ${tx.tokenA}` : `${tx.amountB.toFixed(4)} ${tx.tokenB}`;
      const output = tx.direction==='AtoB' ? `${tx.amountB.toFixed(4)} ${tx.tokenB}` : `${tx.amountA.toFixed(4)} ${tx.tokenA}`;
      ...
      row.innerHTML = `
        ...
        <td style="font-size:.75em">${tx.source==='fogoscan'?'🔵':'🟡'}</td>
    ...
    if (data.transactions.length > 500) {
      const row = document.createElement('tr');
      row.innerHTML = `<td colspan="7" style="text-align:center;color:var(--text-secondary);padding:1rem;">... and ${data.transactions.length-500} more (export CSV)</td>`;
