...
async function fetchCodexVolume(walletAddress, startDate, endDate) {
  ...
  try {
    ...
    if (err.message && err.message.includes('Not authorized')) {
      addLogEntry('warning', `⚠️ Codex: Wallet stats require paid plan`);
      return { error: 'UPGRADE_REQUIRED', message: 'Wallet stats require paid plan. RPC data shown instead.' };
    }
...
function displayCodexComparison(rpcData, codexData) {
  ...
  // If no Codex data, show RPC data only
  if (!codexData || !codexData.wallet) {
    const errorMsg = codexData?.error === 'UPGRADE_REQUIRED' 
      ? 'Codex: Paid plan required for wallet stats'
      : 'Codex API unavailable';
    
    // Show RPC data as fallback
    const rpcTotal = rpcData.totalUsdVolume || 0;
    const rpcSwaps = rpcData.totalSwaps || 0;
    const poolNames = Object.keys(rpcData.poolVolumes || {});
    
    let poolRows = '';
    for (const poolName of poolNames) {
      const pool = rpcData.poolVolumes[poolName];
      poolRows += `
        <tr>
          <td><strong>${poolName}</strong></td>
          <td>$${(pool?.volumeUsd || 0).toFixed(2)}</td>
          <td>-</td>
          <td>-</td>
          <td><span class="method-tag">RPC Only</span></td>
        </tr>
      `;
    }
    ...
    tbody.innerHTML = `
      <tr style="background: rgba(255,107,53,0.1);">
        <td colspan="5" style="text-align: center; padding: 0.75rem; color: var(--text-secondary); font-size: 0.85rem;">
          🔮 ${errorMsg} — Showing RPC data
        </td>
      </tr>
      <tr>
        ...
        <td>-</td>
        <td>-</td>
        <td><span class="method-tag">RPC Calculated</span></td>
      </tr>
      <tr>
        ...
        <td>-</td>
        <td>-</td>
        <td><span class="method-tag">RPC Count</span></td>
      </tr>
      ${poolRows}
    `;
    return;
