...
// ============================================
const FOGOSCAN_API = {
  BASE_URL: 'https://api.fogoscan.com/v1',
  
  async fetchAMMList() {
    const response = await fetch(`${this.BASE_URL}/defi/amm/list`);
    if (!response.ok) throw new Error(`Fogoscan API error: ${response.status}`);
    return response.json();
  },
  
  async fetchAMMMetrics(appId = 'valiant', timeRange = 7) {
    const response = await fetch(`${this.BASE_URL}/defi/amm/metrics?app_id=${appId}&time_range=${timeRange}`);
    if (!response.ok) throw new Error(`Fogoscan API error: ${response.status}`);
    return response.json();
  }
};
