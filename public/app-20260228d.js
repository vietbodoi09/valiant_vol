...
async function fetchValiantCLOB(walletAddress, startTime, endTime) {
  try {
    const pairs = [
      'J7mxBLSz51Tcbog3XsiJTAXS64N46KqbpRGQmd3dQMKp', // FOGO-USDC
      'HULdR8aMSxJAiNJmrTBcfKN4Zq6FgG33AHbQ3nDD8P5E', // FOGO-iFOGO
    ];
    
    const allTrades = [];
    
    for (const pairAddress of pairs) {
      const url = `https://browser-api.valiant.trade/market-api/wallet-trades?wallet=${walletAddress}&limit=1000&provider=codex&pair_address=${pairAddress}`;
      ...
      try {
        ...
        for (const trade of trades) {
          ...
          const tx = {
            ...
            pool: trade.pairAddress === 'J7mxBLSz51Tcbog3XsiJTAXS64N46KqbpRGQmd3dQMKp' ? 'FOGO-USDC' : 'FOGO-iFOGO',
            poolAddress: trade.pairAddress,
            ...
            usdVolume: trade.volumeUsd || 0,
            fogoVolume: (trade.volumeUsd || 0) / fogoPrice,
            signer: trade.owner,
            source: 'clob',
            isValiant: true,
            price: trade.price,
        ...
        addLogEntry('info', `🟣 Valiant CLOB ${pairAddress.slice(0, 8)}: ${trades.length} trades`);
      } catch (e) {
        console.error('CLOB fetch error:', e);
      }
    }
